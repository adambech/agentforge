              ...
              <div className="flex flex-col gap-2">
                {TEMPLATES.map((t, i) => (
                  <button key={i} onClick={() => setDescription(t)}
                    className="text-left text-sm border rounded-xl px-4 py-3 hover:bg-muted transition-colors">
                    {t}
                  </button>
                ))}
              </div>
              ...
              <div className="border rounded-xl overflow-auto">
                {result.filenames.map((f: string) => (
                  <button key={f} onClick={() => setSelectedFile(f)}
                    className={"w-full text-left px-3 py-2 text-xs font-mono hover:bg-muted transition-colors border-b " + (selectedFile === f ? "bg-muted font-bold" : "")}>
                    {f}
                  </button>
                ))}
              </div>
              <div className="col-span-3 border rounded-xl overflow-auto bg-muted p-4">
